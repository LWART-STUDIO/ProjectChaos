...
*/
Shader "SimpleURPToonLitOutlineExample"
{
    Properties
    {
        ...
        [MainTexture]_ShadeMap("_ShadeMap (albedo)", 2D) = "white" {}
        ...
        [Header(Outline)]
        _OutlineWidth("_OutlineWidth (Object Space)", Range(0, 0.1)) = 0.0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            Tags
            {
                // "Lightmode" tag must be "UniversalForward" or "UniversalForwardOnly", in order to render objects in URP.
        ...
        Pass
        {
            ...
            Varyings ShadowCasterPassVertex(Attributes input)
            {
                ...
                //(you can delete this line, this line is just a note) the correct value is false here, else self shadow is not correct
                setting.applyShadowBiasFixToHClipPos = true;
                //important for shadow caster pass, else shadow artifact will appear
        ...
        // DepthNormals pass (для постэффектов)
        Pass
        {
            Name "DepthNormals"
            Tags
            ...
            Varyings DepthNormalsPassVertex(Attributes input)
            {
                VertexShaderWorkSetting setting = GetDefaultVertexShaderWorkSetting();
                setting.isOutline = true; 
                return VertexShaderWork(input, setting);
            ...
            half4 DepthNormalsPassFragment(Varyings input) : SV_TARGET
            {
                return BaseColorAlphaClipTest(input);
            }
            ENDHLSL
