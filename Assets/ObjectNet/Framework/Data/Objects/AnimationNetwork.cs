    ...
    public class AnimationNetwork : NetworkEntity<AnimationInfoPacket, IDataStream> {
        ...
        private void SynchronizeUsingParameters() {
            foreach (AnimatorControllerParameter param in this.animator.parameters) {
                if (!this.animationData.AnimationParameters.ContainsKey(param.nameHash)) {
                    this.animationData.AnimationParameters.Add(param.nameHash, null);
                }
                if (param.type == AnimatorControllerParameterType.Float) {
                    this.FlagUpdated((this.animationData.AnimationParameters[param.nameHash] != null) && ((float)this.animationData.AnimationParameters[param.nameHash] != this.animator.GetFloat(param.nameHash)));
                    this.animationData.AnimationParameters[param.nameHash] = this.animator.GetFloat(param.nameHash);
                } else if (param.type == AnimatorControllerParameterType.Int) {
                    this.FlagUpdated((this.animationData.AnimationParameters[param.nameHash] != null) && ((int)this.animationData.AnimationParameters[param.nameHash] != this.animator.GetInteger(param.nameHash)));
                    this.animationData.AnimationParameters[param.nameHash] = this.animator.GetInteger(param.nameHash);
                } else if (param.type == AnimatorControllerParameterType.Bool) {
                    this.FlagUpdated((this.animationData.AnimationParameters[param.nameHash] != null) && ((bool)this.animationData.AnimationParameters[param.nameHash] != this.animator.GetBool(param.nameHash)));
                    this.animationData.AnimationParameters[param.nameHash] = this.animator.GetBool(param.nameHash);
                } else if (param.type == AnimatorControllerParameterType.Trigger) {
                    this.FlagUpdated((this.animationData.AnimationParameters[param.nameHash] != null) && ((bool)this.animationData.AnimationParameters[param.nameHash] != this.animator.GetBool(param.nameHash)));
                    this.animationData.AnimationParameters[param.nameHash] = this.animator.GetBool(param.nameHash); // Trigger is a ordinary bool                    
                }
            }
        ...
        private void InitializeAnimationData() {
            ...
            this.animationData.AnimationParameters              = new Dictionary<int, object>();
            this.previousAnimationData.PlayingAnimationClips    = new Dictionary<int, AnimationClipPacket>();
            this.previousAnimationData.AnimationParameters      = new Dictionary<int, object>();
        }
                ...
                if (AnimationSyncType.UseController.Equals(this.syncronizationType)) {
                    ...
                    foreach (var animatorParam in this.animationData.AnimationParameters) {
                        ...
                        if (paramFromAnimator.type == AnimatorControllerParameterType.Float) {
                            ...
                            writer.Write<bool>((bool)animatorParam.Value); // Trigger is a ordinary bool
                        }
                    }
                    // Increase duration
        ...
        public override void SynchonizePassive(AnimationInfoPacket data) {
            if (AnimationSyncType.UseController.Equals(this.syncronizationType)) {
                ...
                foreach (int paramNameHash in data.AnimationParameters.Keys.ToArray()) {
                    if (!this.animationData.AnimationParameters.ContainsKey(paramNameHash)) {
                        this.animationData.AnimationParameters.Add(paramNameHash, null);                        
                    }
                    this.animationData.AnimationParameters[paramNameHash] = data.AnimationParameters[paramNameHash];
                }
                this.animationData.AnimationHash = data.AnimationHash;
                this.animationData.AnimationTime = data.AnimationTime;
            ...
            if (isSenderPaused == false) {
                ...
                if (AnimationSyncType.UseController.Equals(this.syncronizationType)) {
                    ...
                    for (int indexCount = 0; indexCount < this.animationData.Count; indexCount++) {
                        ...
                        this.animationData.AnimationParameters[paramNameHash] = paramValue;
                    }
