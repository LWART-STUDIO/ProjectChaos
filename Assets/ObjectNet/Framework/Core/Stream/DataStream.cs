    ...
    public class DataStream : IDataStream, INetworkStream {
        ...
        private void Initialize() {
            if ( this.streamMap.Count == 0) {
                ...
                this.streamMap.Add(typeof(Int64),       new Tuple<IData, IData>(new Int64Stream(),      new Int64Stream()));
                this.streamMap.Add(typeof(UInt64),      new Tuple<IData, IData>(new UInt64Stream(),     new UInt64Stream()));
                ...
                this.streamMap.Add(typeof(sbyte),       new Tuple<IData, IData>(new SByteStream(),      new SByteStream()));
                ...
                this.allowedTypes.Add(typeof(Int16));
                this.allowedTypes.Add(typeof(UInt16));
                this.allowedTypes.Add(typeof(Int32));
                this.allowedTypes.Add(typeof(UInt32));
                this.allowedTypes.Add(typeof(Int64));
                this.allowedTypes.Add(typeof(UInt64));
                this.allowedTypes.Add(typeof(float));
                this.allowedTypes.Add(typeof(double));
                this.allowedTypes.Add(typeof(bool));
                this.allowedTypes.Add(typeof(string));
                this.allowedTypes.Add(typeof(Color));
                this.allowedTypes.Add(typeof(sbyte));
                this.allowedTypes.Add(typeof(byte));
                this.allowedTypes.Add(typeof(byte[]));
                this.allowedTypes.Add(typeof(Vector2));
                this.allowedTypes.Add(typeof(Vector3));
                this.allowedTypes.Add(typeof(Quaternion));

                // Register global stream classes
                foreach (var streamFactory in DataStream.streamFactory) {
                    if (!this.streamMap.ContainsKey(streamFactory.Key)) {
                    ...
                    if (!this.allowedTypes.Contains(streamFactory.Key)) {
                        this.allowedTypes.Add(streamFactory.Key);
                    }
                }
            ...
            if (reader != null) {
                ...
                if (reader is IReader) {
                    ...
                    if (dataType == typeof(Int16))
                        ...
                        result = (reader as IReader).Read<Int64>(this.dataBuffer, ref this.bufferIndex, dataType);
                    else if (dataType == typeof(UInt64))
                        result = (reader as IReader).Read<UInt64>(this.dataBuffer, ref this.bufferIndex, dataType);
                        ...
                        result = (reader as IReader).Read<sbyte>(this.dataBuffer, ref this.bufferIndex, dataType);
                    else if (dataType == typeof(byte[]))
                        result = (reader as IReader).Read<byte[]>(this.dataBuffer, ref this.bufferIndex, dataType);
        ...
        public bool IsValidType<T>() {
            // Check if some new stream was registered
            this.Initialize();
            return this.allowedTypes.Contains(typeof(T));
        }
        ...
        public bool IsValidType(Type dataType) {
            // Check if some new stream was registered
            this.Initialize();
            return this.allowedTypes.Contains(dataType);
        }
        ...
        public Type GetType(ushort index) {
            // Check if some new stream was registered
            this.Initialize();
            return this.allowedTypes[index];
        }
        ...
        public ushort GetTypeIndex(Type type) {
            // Check if some new stream was registered
            this.Initialize();
            return (ushort)this.allowedTypes.IndexOf(type);
        }
        ...
        public ushort GetTypeIndex<T>() {
            // Check if some new stream was registered
            this.Initialize();
            return (ushort)this.allowedTypes.IndexOf(typeof(T));
        }
