...
half4 FragVelocitySetup(VaryingsDefault i) : SV_Target
{
    ...
    float2 v = SAMPLE_TEXTURE2D(_MotionVectorTexture, sampler_LinearClamp, i.uv).xy;
    ...
    half d = LinearizeDepth(SAMPLE_TEXTURE2D(_CameraDepthTexture, sampler_PointClamp, i.uv).r);
...
half4 FragTileMax1(VaryingsDefault i) : SV_Target
{
    ...
    half2 v1 = SAMPLE_TEXTURE2D(_MainTex, sampler_LinearClamp, i.uv + d.xy).rg;
    half2 v2 = SAMPLE_TEXTURE2D(_MainTex, sampler_LinearClamp, i.uv + d.zy).rg;
    half2 v3 = SAMPLE_TEXTURE2D(_MainTex, sampler_LinearClamp, i.uv + d.xw).rg;
    half2 v4 = SAMPLE_TEXTURE2D(_MainTex, sampler_LinearClamp, i.uv + d.zw).rg;
...
half4 FragTileMaxV(VaryingsDefault i) : SV_Target
{
    ...
    for (int ix = 0; ix < _TileMaxLoop; ix++)
    {
        ...
        for (int iy = 0; iy < _TileMaxLoop; iy++)
        {
            ...
            vo = MaxV(vo, SAMPLE_TEXTURE2D(_MainTex, sampler_LinearClamp, uv).rg);
        }
    }
...
half4 FragNeighborMax(VaryingsDefault i) : SV_Target
{
    ...
    half2 v1 = SAMPLE_TEXTURE2D(_MainTex, sampler_LinearClamp, i.uv - d.xy).rg;
    half2 v2 = SAMPLE_TEXTURE2D(_MainTex, sampler_LinearClamp, i.uv - d.wy).rg;
    half2 v3 = SAMPLE_TEXTURE2D(_MainTex, sampler_LinearClamp, i.uv - d.zy).rg;

    half2 v4 = SAMPLE_TEXTURE2D(_MainTex, sampler_LinearClamp, i.uv - d.xw).rg;
    half2 v5 = SAMPLE_TEXTURE2D(_MainTex, sampler_LinearClamp, i.uv).rg * cw;
    half2 v6 = SAMPLE_TEXTURE2D(_MainTex, sampler_LinearClamp, i.uv + d.xw).rg;

    half2 v7 = SAMPLE_TEXTURE2D(_MainTex, sampler_LinearClamp, i.uv + d.zy).rg;
    half2 v8 = SAMPLE_TEXTURE2D(_MainTex, sampler_LinearClamp, i.uv + d.wy).rg;
    half2 v9 = SAMPLE_TEXTURE2D(_MainTex, sampler_LinearClamp, i.uv + d.xy).rg;
...
{
    half3 v = SAMPLE_TEXTURE2D_LOD(_VelocityTex, sampler_LinearClamp, uv, 0).xyz;
...
half4 FragReconstruction(VaryingsMultitex i) : SV_Target
{
    ...
    const half4 c_p = SAMPLE_TEXTURE2D(_MainTex, sampler_LinearClamp, i.uv0);
    ...
    const half2 v_max = SAMPLE_TEXTURE2D(_NeighborMaxTex, sampler_LinearClamp, i.uv1 + JitterTile(i.uv1)).xy;
    ...
    UNITY_LOOP while (t > dt * 0.25)
    {
        ...
        // Color sample
        const half3 c = SAMPLE_TEXTURE2D_LOD(_MainTex, sampler_LinearClamp, uv0, 0).rgb;
