...
namespace com.onlineobject.objectnet {
    ...
    public class PositionNetwork : NetworkEntity<Vector3, IDataStream> {
        ...
        private float GetObjectSpeed() {
            float result = (Vector3.Distance(((PositionReference.UseGlobal.Equals(this.positionReference)) ? this.transformElement.position : this.transformElement.localPosition), this.previousPositionForSpeed) / this.executionDeltaTime);
            this.previousPositionForSpeed = ((PositionReference.UseGlobal.Equals(this.positionReference)) ? this.transformElement.position : this.transformElement.localPosition);
            return result;
        }
